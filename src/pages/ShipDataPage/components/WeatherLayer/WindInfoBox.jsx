...
function WindInfoBoxContent({ grid, selectedTime, latlng, minTime, maxTime }) {
    if (!grid || !latlng || !grid.ts) return null;
    ...
    return (
        ...
        >
            ...
            <Typography variant="body2">
                <strong>Speed:</strong>{" "}
                {speed.toFixed(1)} m/s ({speedKnots.toFixed(1)} kt)
            </Typography>
            ...
            {gust != null && (
                <Typography variant="body2">
                    <strong>Gust:</strong>{" "}
                    {gust.toFixed(1)} m/s ({gustKnots.toFixed(1)} kt)
                </Typography>
            )}
            ...
            <Typography variant="body2">
                <strong>Time:</strong>{" "}
                {new Date(grid.ts[timeIndex]).toLocaleString()}
            </Typography>
        </Box>
    );
}
...
export function WindInfoBox({
    ...
    // Update content on click or time change
    useEffect(() => {
